...
// Image Settings
#define PIXFORMAT_SHOT PIXFORMAT_JPEG

#define PIXFORMAT_MOTION PIXFORMAT_GRAYSCALE
...
void RebootCamera(pixformat_t format, framesize_t fsize) {
  ...
  delay(300);
  RestartCamera(PIXFORMAT_MOTION, FRAME_SIZE_MOTION);   // restart camera in motion mode
  delay(300);
  ...
  if (format == PIXFORMAT_SHOT) RestartCamera(PIXFORMAT_SHOT, fsize);                 // if jpg mode required restart camera again
}
...
bool cameraImageSettings(framesize_t fsize, bool isMotionDetect) {
  ...
  if (isMotionDetect && !isDetectMode) {
    RestartCamera(PIXFORMAT_MOTION, FRAME_SIZE_MOTION);
    isDetectMode = true;
    // Serial.println("PIXFORMAT_GRAYSCALE");
    Serial.println("PIXFORMAT_GRAYSCALE");
  }
  if (!isMotionDetect && isDetectMode) {
    RestartCamera(PIXFORMAT_SHOT, fsize);
    isDetectMode = false;
    cameraSetAdjust();
...
bool capture_still(framesize_t oldfsize) {
  ...
  for (int y = 0; y < H; y++) {
    for (int x = 0; x < W; x++) {
      uint16_t currentBlock = temp_frame[y][x] / (BLOCK_SIZE * BLOCK_SIZE+1 );    // average pixel value in block
